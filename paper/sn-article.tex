...
We also measured the inference time and total trainable parameters of deep approaches using the Google Colab environment. DistilBERT performs slower than CNN and Seq2Seq approaches with the same initial seed but is faster than approaches using LLMs. The inference time and total parameters of each model on the TREC and SST2 testing sets are presented in Table~\ref{tab:inference-result} and Table~\ref{tab:parameters-result}. 
...
\textbf{Model}  & \textbf{TREC}  & \textbf{SST2}\\ \midrule
BiGRU Seq2Seq                      & 0 &  20,254,722                     \\ \midrule
CNN + random embedding             & 0 & 4,802,702                  \\
CNN + fastText (freezed)           & 0 & 360,902                     \\
CNN + fastText (trainable)         & 0 & 4,802,702                    \\ \midrule
CNN + LSTM + random embedding      & 0 & 6,568,012                   \\
CNN + BiGRU + random embedding     & 0 & 6,652,492                   \\
CNN + BiGRU + fastText (freezed)   & 0 & 986,892                  \\
CNN + BiGRU + fastText (trainable) & 0 & 9,870,492                 \\ \midrule
Finetuned BERT                     & 0 & 108,311,810                  \\
Finetuned XLM-RoBERTa              & 0 & 278,045,186                  \\ \midrule
Finetuned DistilBERT\footnotemark[1]               & 0 & 65,783,042                    \\ \bottomrule
\end{tabular*}
\footnotetext[1]{Our proposed method}
\end{table}
